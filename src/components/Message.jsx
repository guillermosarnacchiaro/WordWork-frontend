...
export default function Message({ mensaje }) {
  return (
    <div className="message-row" style={{ display: 'flex', justifyContent: mensaje.sent ? 'flex-end' : 'flex-start', marginBottom: '4px', padding: '0 clamp(8px, 4vw, 60px)' }}>
      <div className={`message-bubble ${mensaje.sent ? 'message-bubble--sent' : 'message-bubble--received'}`} style={{
        ...
        color: mensaje.sent ? '#ffffff' : 'var(--text-primary)', fontSize: '14.2px', lineHeight: '1.45',
        border: mensaje.sent ? 'none' : '1px solid var(--border)',
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: '3px', minHeight: '16px', marginTop: '1px' }}>
          <span style={{ color: mensaje.sent ? 'rgba(255,255,255,.7)' : 'var(--text-muted)', fontSize: '10.5px', lineHeight: 1 }}>{mensaje.time}</span>
          {mensaje.sent && (mensaje.read
            ? <CheckCheck size={16} strokeWidth={2.25} color="#bfdbfe" aria-label="Leído" />
            : mensaje.delivered
              ? <CheckCheck size={16} strokeWidth={2.25} color="#bfdbfe" aria-label="Entregado" />
              : <Check size={16} strokeWidth={2.25} color="#bfdbfe" aria-label="Enviado" />)}
        </div>
      </div>
    </div>
  )
